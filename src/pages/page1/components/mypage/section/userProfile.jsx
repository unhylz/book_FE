...
function UserProfile({ userData, handleImageChange }) {
  ...
  async function handleImageChange(event) {
    ...
    try {
      const response = await fetch('http://localhost:3001/users/{user-id}/mypage', { 
        method: 'POST',
        body: formData,
      });
  ...
  return (
    ...
    <div className='userdata-container'>
      <div className='last-tier'><strong>Season 1: Master</strong></div>
      ...
      <div className='message-edit'>
        <div className='message-box'><h5>{message}</h5></div>
        <button className="edit-button2" onClick={handleEditButtonClick}>
          <img src={flag} alt="flagIcon"/>
          메시지 수정
        </button>
        {/* 모달 컴포넌트 */}
      <MypageModal isOpen={isModalOpen} onClose={handleModalClose} onSubmit={handleModalSubmit} />
      </div>
